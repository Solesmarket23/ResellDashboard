  ...
  return (
    <div className={`flex-1 ${currentTheme.colors.background} p-8`}>
      ...
      {resolvedUserId ? (
        ...
        <div className={`mb-6 ${
          isNeon 
            ? 'bg-gradient-to-br from-blue-500/10 via-indigo-500/10 to-purple-500/10 border-2 border-blue-500/30' 
            : 'bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200'
        } rounded-xl p-6 shadow-lg`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className={`flex items-center justify-center w-12 h-12 rounded-full ${
                isNeon ? 'bg-blue-500/20 ring-2 ring-blue-500/50' : 'bg-blue-100 ring-2 ring-blue-300'
              }`}>
                <Package className={`w-6 h-6 ${isNeon ? 'text-blue-400' : 'text-blue-600'}`} />
              </div>
              <div>
                <h3 className={`text-lg font-bold ${isNeon ? 'text-white' : 'text-gray-900'}`}>
                  Import StockX Sales
                </h3>
                <p className={`text-sm mt-1 ${isNeon ? 'text-gray-400' : 'text-gray-600'}`}>
                  Sign in to import your StockX sales history
                </p>
              </div>
            </div>
            <button
