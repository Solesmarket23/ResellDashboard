...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!retrackResponse.ok) {
      const retrackError = await retrackResponse.json();
      console.log(`⚠️ Retrack failed: ${retrackResponse.status}`, retrackError);
    } else {
      console.log(`✅ Retrack successful for: ${trackingNumber}`);
    }
    
    // Step 2: Get the updated tracking data
    ...
    return NextResponse.json({
      ...
      status: tracking.tag,
      eddInfo,
      message: 'Force refresh completed - check EDD data'
    });
