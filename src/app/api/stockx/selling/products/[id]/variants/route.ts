...
export async function GET(
  ...
  try {
    ...
    console.log(`🏷️ Fetching variants via selling API for: ${productId}`);
    ...
    // Try the selling API pattern since /selling/listings works
    const variantsUrl = `https://api.stockx.com/v2/selling/products/${productId}/variants`;
    ...
    if (!response.ok) {
      ...
      console.error('❌ Failed to fetch variants via selling API:', {
      ...
      return NextResponse.json(
        { 
          error: 'Failed to fetch product variants via selling API',
          status: response.status,
          statusText: response.statusText,
          productId: productId,
          url: variantsUrl,
    ...
    console.log(`✅ Selling API variants response:`, {
    ...
    console.error('❌ Error fetching variants via selling API:', error);
    return NextResponse.json(
      { 
