...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔄 CLEAR AND RE-PARSE ORDER: ${orderNumber}`);

    // First, clear any existing purchase records for this order
    try {
      const existingPurchases = await getDocuments('purchases');
      const orderPurchases = existingPurchases.filter((p: any) => p.orderNumber === orderNumber);
      
      console.log(`🗑️ Found ${orderPurchases.length} existing records for order ${orderNumber}`);
      
      for (const purchase of orderPurchases) {
        if (purchase.id) {
          console.log(`🔥 Deleting existing purchase record: ${purchase.id}`);
          await deleteDocument('purchases', purchase.id);
        }
      }
      
      console.log(`✅ Cleared ${orderPurchases.length} existing records`);
    } catch (error) {
      console.warn('⚠️ Error clearing existing records (continuing anyway):', error);
    }
    ...
    return NextResponse.json({
      ...
      message: `Successfully cleared and re-parsed order ${orderNumber} with updated logic`
    });

  } catch (error) {
    console.error('Error clearing and re-parsing order:', error);
    return NextResponse.json(
      { 
        error: 'Failed to clear and re-parse order',
...
export async function GET() {
  return NextResponse.json({
    message: 'Use POST to clear and re-parse a specific order',
