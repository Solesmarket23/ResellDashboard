...
exports.OrderConfirmationParser = void 0;
exports.parseOrderEmailFile = parseOrderEmailFile;
exports.parseGmailApiMessage = parseGmailApiMessage;
exports.orderInfoToDict = orderInfoToDict;
...
class OrderConfirmationParser {
    constructor(debug = false) {
        this.supportedMerchants = ['stockx'];
        this.debug = false;
        this.debug = debug;
    }
    /**
     ...
     */
    parseEmailMessage(emailContent) {
        ...
        if (subjectMatch)
            orderInfo.email_subject = this.decodeHeader(subjectMatch[1]);
        if (dateMatch)
            orderInfo.email_date = dateMatch[1];
        if (fromMatch)
            orderInfo.sender = fromMatch[1];
     ...
     */
    parseStockXEmail(htmlContent, orderInfo) {
        // Clean HTML and extract text
        const $ = cheerio.load(htmlContent);
        const textContent = ($('body').text && $('body').text()) || ($.root && $.root().text && $.root().text()) || '';
        ...
        const isShippingEmail = shippingSubjectPatterns.some(pattern => emailSubject.includes(pattern) || htmlContent.includes(pattern));
     ...
     */
    extractStockXProductInfo(htmlContent, textContent, orderInfo) {
        var _a;
        ...
        for (let i = 0; i < sizePatterns.length; i++) {
            ...
            if (match) {
                ...
                if (this.debug) {
                    ...
                    const idx = (_a = match.index) !== null && _a !== void 0 ? _a : 0;
                ...
                const isCssKeyword = cssKeywords.some(keyword => size.toLowerCase().includes(keyword.toLowerCase()));
     ...
     */
    tryFallbackSizeExtraction(htmlContent, textContent, orderInfo) {
        ...
        for (const pattern of aggressivePatterns) {
            ...
            for (const match of matches) {
                ...
                if (size.length < 1 || size.length > 25)
                    continue;
     ...
     */
    getDefaultSizeForProduct(productName) {
        if (!productName)
            return null;
     ...
     */
    extractStockXProductImage(htmlContent, orderInfo) {
        const $ = cheerio.load(htmlContent);
     ...
     */
    extractStockXPricing(htmlContent, textContent, orderInfo) {
        // Purchase Price
        const pricePatterns = [
     ...
     */
    extractStockXDelivery(htmlContent, textContent, orderInfo) {
     ...
     */
    extractStockXPurchaseDate(htmlContent, textContent, orderInfo) {
     ...
     */
    extractStockXTrackingInfo(htmlContent, textContent, orderInfo) {
     ...
     */
    extractStockXOrderNumber(textContent, orderInfo) {
        const orderPatterns = [
     ...
     */
    isXpressOrderNumber(orderNumber) {
     ...
     */
    isRegularOrderNumber(orderNumber) {
...
exports.OrderConfirmationParser = OrderConfirmationParser;
// Helper functions for external use
/**
 * Parse an order confirmation email from a file
 */
function parseOrderEmailFile(filePath) {
    const fs = require('fs');
 ...
 */
function parseGmailApiMessage(gmailMessage, debug = false) {
    var _a, _b;
    if (debug) {
    ...
    if (gmailMessage.payload) {
        ...
        const decodePartBody = (part) => {
            var _a;
            const b64 = ((_a = part === null || part === void 0 ? void 0 : part.body) === null || _a === void 0 ? void 0 : _a.data) || '';
            if (!b64)
                return '';
            ...
            const charset = ((charsetMatch === null || charsetMatch === void 0 ? void 0 : charsetMatch[1]) || 'utf-8').toLowerCase();
            const looksQuotedPrintable = transferEncoding.toLowerCase() === 'quoted-printable' || /=\r?\n|=3D/.test(raw);
            const qpDecodeToBuffer = (qp) => {
    ...
    const headers = ((_a = gmailMessage.payload) === null || _a === void 0 ? void 0 : _a.headers) || [];
    for (const header of headers) {
        const name = ((_b = header.name) === null || _b === void 0 ? void 0 : _b.toLowerCase()) || '';
 ...
 */
function orderInfoToDict(orderInfo) {
    return {
